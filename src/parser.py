...
def p_x(p):
    ''' x : itemlist x | important x | para x | spara x | address x | mobj x | inftable x | comment x | abstract x
        | itemlist | important | para | spara | address | mobj | inftable | comment | abstract'''
def p_z(p):
    ''' z : sec z | ssec z | sec | ssec'''
def p_sec(p):
...
def p_i(p):
    ''' i : TEXTO i | emphasis i | link i | email i | author i | comment i | itemlist i | important i | address i | mobj i | inftable i
        | TEXTO | emphasis | link | email | author | comment | itemlist | important | address | mobj | inftable'''   
def p_important(p):
...
def p_k(p):
    ''' k : TEXTO k | link k | emphasis k | comment k | TEXTO | link | emphasis | comment'''
...
def p_l(p):
    ''' l : vobj l | iobj l | vobj | iobj'''
def p_iobj(p):
    ''' iobj : APERTURA_IMAGENOBJECT info idata CIERRE_IMAGENOBJECT | APERTURA_IMAGENOBJECT idata CIERRE_IMAGENOBJECT'''
def p_idata(p):
    ''' idata : APERTURA_IMAGEDATA'''
def p_vobj(p):
    ''' vobj : APERTURA_VIDEOOBJECT info vdata CIERRE_VIDEOOBJECT | APERTURA_VIDEOOBJECT vdata CIERRE_VIDEOOBJECT'''
def p_vdata(p):
...
def p_tfood(p):
    ''' tfood : APERTURA_TFOOD oo CIERRE_TFOOD'''
def p_tbody(p):
...
def p_error(p):
    ...
    print ("Eror en la linea"+str(p.lineno))
